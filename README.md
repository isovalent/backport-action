...
Prefix added to every target branch selected from labels or the `target_branches` input.
For example, the prefix `stable/` changes `release-1.0` to `stable/release-1.0`.

### `target_branch_suffix`

Default: `''` (not set)

Suffix added to every target branch selected from labels or the `target_branches` input.
For example, the suffix `-backport` changes `release-1.0` to `release-1.0-backport`.
